...
export function DailyPlannerModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-in fade-in duration-200">
      <div className="w-full max-w-3xl rounded-3xl bg-slate-900 border border-slate-800 p-6 shadow-2xl space-y-6 max-h-[92vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800/80 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-slate-400">Hoàn thành hôm nay</p>
          ...
          <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800/80 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-slate-400">Thời gian tập trung</p>
              <p className="text-base font-bold text-slate-100">
          ...
          <div className="p-3.5 rounded-2xl bg-slate-950/80 border border-slate-800/80 flex items-center gap-3">
            ...
            <div>
              <p className="text-[11px] text-slate-400">Đang dở dang</p>
              <p className="text-base font-bold text-slate-100">
        ...
        <div className="space-y-2 text-xs">
          ...
          <textarea
            ...
            className="w-full px-3.5 py-2.5 rounded-2xl bg-slate-950 border border-slate-800 text-slate-100 placeholder-slate-500 outline-none focus:border-purple-500 resize-none"
          />
        </div>

        {/* Section 3: AI Provider Choice & Trigger Button */}
        <div className="flex items-center justify-between gap-4 p-3 rounded-2xl bg-purple-950/20 border border-purple-900/40 text-xs">
          <div className="flex items-center gap-2">
            <Bot className="w-4 h-4 text-purple-400" />
            <span className="text-slate-300 font-medium">Model LLM:</span>
            <select
              value={selectedProvider}
              onChange={(e) => setSelectedProvider(e.target.value as AIProviderType)}
              className="px-2.5 py-1.5 rounded-xl bg-slate-900 border border-slate-700 text-slate-200 outline-none"
            >
              <option value={AIProviderType.GEMINI}>Google Gemini 1.5/2.0 Flash</option>
              <option value={AIProviderType.OPENAI}>OpenAI GPT-4o Mini</option>
              <option value={AIProviderType.CLAUDE}>Anthropic Claude 3.5</option>
            </select>
          </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="flex items-center gap-2 px-5 py-2 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:opacity-95 text-white font-bold shadow-lg shadow-purple-600/30 disabled:opacity-50 transition"
          >
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-3 duration-300">
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="space-y-2 max-h-56 overflow-y-auto">
                {tasksToApply.map((t, idx) => (
                  <div
                    key={idx}
                    className="p-3 rounded-xl bg-slate-950/80 border border-slate-800 flex items-start justify-between gap-3 hover:border-indigo-500/40 transition"
                  >
            ...
            <div className="pt-2 flex justify-end">
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:opacity-95 text-white font-bold text-xs shadow-lg shadow-emerald-600/30 disabled:opacity-50 transition"
              >
